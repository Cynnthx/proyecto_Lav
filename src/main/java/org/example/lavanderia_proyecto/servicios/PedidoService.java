...
public class PedidoService {
    ...
    public Double calcularImporte (Integer pedidoId){
        Pedido pedido = pedidoRepositorio.findById(pedidoId).orElseThrow(()-> new RuntimeException("No se encuentra el pedido"));

        List<PedidoPrendaCatalogo> detalles = pedidoPrendaCatalogoRepositorio.findByPedidoId(pedidoId);

        Double totalImporte = 0.0;

        for (PedidoPrendaCatalogo detalle: detalles){
